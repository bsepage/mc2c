...
int main(int argc, char *argv[])
{
	...
	if (!va1) { LOGERROR("%s", "failed to find virtual pointer\n"); }
	LOGINFO("va1= %p\n", va1);

	void *va2 = m.find_da(array<bool, 5> {{0, 0, 0, 1, 0}});
	if (!va2) { LOGERROR("%s", "failed to find virtual pointer\n"); }
	LOGINFO("va2= %p\n", va2);
	
	void *va3 = m.find_da(array<bool, 5> {{0, 0, 0, 1, 1}});
	if (!va3) { LOGERROR("%s", "failed to find virtual pointer\n"); }
	LOGINFO("va3= %p\n", va3);
	
	void *va4 = m.find_da(array<bool, 5> {{0, 0, 1, 0, 0}});
	if (!va4) { LOGERROR("%s", "failed to find virtual pointer\n"); }
	LOGINFO("va4= %p\n", va4);
	
	void *va5 = m.find_da(array<bool, 5> {{0, 0, 1, 0, 1}});
	if (!va5) { LOGERROR("%s", "failed to find virtual pointer\n"); }
	LOGINFO("va5= %p\n", va5);
	
	void *va6 = m.find_da(array<bool, 5> {{0, 0, 1, 1, 0}});
	if (!va6) { LOGERROR("%s", "failed to find virtual pointer\n"); }
	LOGINFO("va6= %p\n", va6);
	
	void *va7 = m.find_da(array<bool, 5> {{0, 0, 1, 1, 1}});
	if (!va7) { LOGERROR("%s", "failed to find virtual pointer\n"); }
	LOGINFO("va7= %p\n", va7);
